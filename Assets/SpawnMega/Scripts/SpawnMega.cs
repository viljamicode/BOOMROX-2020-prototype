...
public class SpawnMega : MonoBehaviour
{
	...
	// This is to prevent spawn overlapping
	private bool canTeleport = true;
	void OnCollisionEnter2D(Collision2D coll)
	{
		if (coll.gameObject.tag == "Enemy" || coll.gameObject.tag == "Player" || coll.gameObject.tag == "Default" && canTeleport)
		{
			coll.gameObject.transform.position = coll.gameObject.transform.position + new Vector3(-2, 0);
			canTeleport = false;
		}
	}
